...
export default function Dashboard() {
  ...
  const filteredAndSortedData = dummyURLChecks
    .filter(check => {
  ...
  const stats = {
    total: dummyURLChecks.length,
    blocked: dummyURLChecks.filter(c => c.actionTaken === 'blocked').length,
    suspicious: dummyURLChecks.filter(c => c.riskScore > 50 && c.actionTaken === 'allowed').length
  };
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <div className="flex items-center space-x-2">
                        <span className={`font-medium ${
                          check.riskScore >= 70 ? 'text-red-600' : 
